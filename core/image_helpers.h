  ...
  namespace {
    ...
    template <class... DestImageType>
      struct __assign<std::tuple<DestImageType...>> { NOMEMALIGN
        ...
        const ssize_t index;
        template <class ImageType>
          FORCE_INLINE void operator() (ImageType& x) { do_apply (__assign<DestImageType...> (axis, index), x); }
      };
    ...
    template <class... DestImageType>
      struct __max_axis<std::tuple<DestImageType...>> { NOMEMALIGN
        ...
        size_t& axis;
        template <class ImageType>
          FORCE_INLINE void operator() (ImageType& x) { do_apply (__max_axis<DestImageType...> (axis), x); }
      };
      ...
      struct __assign_pos_axis_range { NOMEMALIGN
        template <class... DestImageType>
          FORCE_INLINE void to (DestImageType&... dest) const {
            ...
            do_apply (__max_axis<DestImageType...> (last_axis), std::tie (ref, dest...));
            for (size_t n = from_axis; n < last_axis; ++n)
              do_apply (__assign<DestImageType...> (n, __get_index (ref, n)), std::tie (dest...));
          }
        const ImageType& ref;
    ...
    template <class ImageType, typename IntType>
      struct __assign_pos_axes { NOMEMALIGN
        template <class... DestImageType>
          FORCE_INLINE void to (DestImageType&... dest) const {
            for (auto a : axes)
              do_apply (__assign<DestImageType...> (a, __get_index (ref, a)), std::tie (dest...));
          }
        const ImageType& ref;
